-- Travel Buddy production hardening (applied remotely as travel_buddy_security_hardening)
-- Apply only to verified project: travel-buddy-production (farnjmgwcayvkzuaoifk).
-- Do not apply to aboss-production, ai-invoicing-app-production, or aleya-logo-creator.
...
create policy templates_insert_owner on public.trip_templates
  for insert with check ((select auth.uid()) = owner_id);
